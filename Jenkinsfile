        ...
        withCredentials([file(credentialsId: SERVER_KEY_CREDENTALS_ID, variable: 'server_key_file')]) {
            ...
            stage('Deploye Code') {
                ...
                if (isUnix()) {
                   ...
                   rmsg = bat returnStdout: true, script: "\"C:\\Users\\GeekSoft - 5\\AppData\\Local\\Programs\\Salesforce CLI\\bin\\sfdx\" force:auth:logout --targetusername ${SF_USERNAME} -p & \"C:\\Program Files\\Salesforce CLI\\bin\\sfdx\" force:auth:jwt:grant --clientid ${SF_CONSUMER_KEY} --instanceurl ${SF_INSTANCE_URL} --username ${SF_USERNAME} --jwtkeyfile \"${server_key_file}\" & \"C:\\Program Files\\Salesforce CLI\\bin\\sfdx\" force:source:deploy --manifest manifest/package.xml -u ${SF_USERNAME}"
